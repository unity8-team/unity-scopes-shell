...
RegistrySpawner::RegistrySpawner(const QString &runtimeConfig)
{
    qputenv("UNITY_SCOPES_CONFIG_DIR", m_tempDir.path().toUtf8());
    qputenv("TEST_DESKTOP_FILES_DIR", "");

    QDir endpointdir(QFileInfo(runtimeConfig).dir());
    ...
    qputenv("UNITY_SCOPES_RUNTIME_PATH", runtimeConfig.toUtf8());
